...
class GraphStructure():
  ...
  # Given poses, outputs a score tensor indicating how much weight nodes are assigned, per pose
  # Returns on same device as the poses passed in
  def get_interpolation_coordinates(self, poses, force=False):
    ...
    assert(nx.shape == ny.shape == shape[:2])

    node_pos = self.node_positions.to(poses.device)

    # Flatten indices tensors to get 1D tensor for use in index_select
    # Get indices this way as node_positions is flat with positions sorted by x (and within an x, sorted by y)
    ...
    if force: return(top_left_idx, top_right_idx, bottom_right_idx, bottom_left_idx)
    ...
    assert(bottom_left_score.shape == (shape[0], shape[1]))
    ...
    assert(scores.shape == (shape[0], shape[1], self.grid['n_x']*self.grid['n_y']))
    ...
    detailed = True
    if detailed: return scores, [bottom_left_score, top_left_score, top_right_score, bottom_right_score]
    # Score is returned on same device as poses
    return scores
