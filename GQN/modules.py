...
class Pool(nn.Module):
    ...
    def forward(self, x, v):
        ...
        # Broadcast
        if glo.IMG_SIZE == 64: v = v.view(v.size(0), 7, 1, 1).repeat(1, 1, 16, 16)
        elif glo.IMG_SIZE == 16:  v = v.view(v.size(0), 7, 1, 1).repeat(1, 1, 4, 4)
...
class InferenceCore(nn.Module):
    ...
    def forward(self, x_q, v_q, r, c_e, h_e, h_g, u):
        if glo.IMG_SIZE == 64: x_q = self.downsample_x(x_q)
        else: pass
